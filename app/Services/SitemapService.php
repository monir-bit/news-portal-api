...
class SitemapService
{
     ...
     *
     * @return list<array{slug: string, url: string, lastmod: string}>
     */
    public function getPosts(int $page = 1, int $perPage = self::POSTS_PER_PAGE): array
    {
        ...
        $news = News::query()
            ...
            ->select(['id', 'slug_key', 'category_id', 'created_at', 'updated_at'])
     ...
     *
     * @return list<array{slug: string, url: string}>
     */
     ...
     *
     * @return list<array{slug: string, url: string, title: string, date: string}>
     */
    public function getNewsLast48Hours(): array
    {
        ...
        $news = News::query()
            ...
            ->select(['id', 'slug_key', 'title', 'category_id', 'created_at'])
            ->with('category.parentRecursive')
