...
use App\Http\Requests\QuestionAnswerStoreRequest;
use App\Http\Requests\QuestionParticipationRequest;
...
class QuestionController extends Controller
{
    public function getQuestion(string $categorySlug)
    {
        $category = Category::query()
            ...
            ->with([
                'questions' => fn ($q) => $q->activeNow()->with(['options:id,question_id,option_text']),
            ])
            ->firstOrFail();
    ...
    {
        $category = Category::query()->select(['id'])->where('slug', $categorySlug)->firstOrFail();
