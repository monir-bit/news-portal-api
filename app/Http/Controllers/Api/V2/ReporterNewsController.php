...
class ReporterNewsController extends Controller
{
        ...
        $mapped = $paginator->getCollection()
            ...
            ->map(function ($rn) {
                ...
                if ($n->category && $n->slug_key) {
                    $url = '/'.$this->buildCategoryPath($n->category).'/'.$n->slug_key;
                }
     ...
     */
    private function expandReporterLocationOptions(Reporter $reporter): array
    {
        $options = [];
    ...
    /**
     * Build the "/segment/segment/.../slug" category path for a news item.
     *
     * Inlined from App\Services\CategoryPathService::build() per v2 convention
     * (no separate Service classes). Walks the same relation the app eager-loads
     * here (`category.parent`, one level) so this mirrors v1 behavior exactly,
     * including falling back to lazy-loading `parentRecursive` for deeper trees.
     */
    private function buildCategoryPath(Category $category): string
    {
        ...
        while ($current) {
            ...
            if ($current->relationLoaded('parentRecursive')) {
                $current = $current->parentRecursive;
            } elseif ($current->relationLoaded('parent')) {
